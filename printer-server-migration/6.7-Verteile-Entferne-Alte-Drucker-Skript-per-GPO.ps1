# Script zum Verteilen des Client-Scripts zum Entfernen alter Drucker per GPO
# Erstellt eine GPO, die das Script bei jedem Anmelden ausführt

param(
    [Parameter(Mandatory=$false)]
    [string]$GPOName = "Drucker - Entferne Drucker alter Server",
    [Parameter(Mandatory=$false)]
    [string]$ScriptName = "6.6-Client-Entferne-Drucker-von-Alten-Servern.ps1",
    [Parameter(Mandatory=$false)]
    [string[]]$OldPrintServers = @("<PRINT-SERVER-1>-01", "<PRINT-SERVER-2>", "<PRINT-SERVER-3>")
)

Write-Host "=== Verteile Client-Script zum Entfernen alter Drucker per GPO ===" -ForegroundColor Cyan
Write-Host "GPO-Name: $GPOName" -ForegroundColor Yellow
Write-Host "Script-Name: $ScriptName" -ForegroundColor Yellow
Write-Host "Alte Druckerserver: $($OldPrintServers -join ', ')" -ForegroundColor Yellow
Write-Host ""
...
# Erstelle eine Konfigurationsdatei mit den alten Servernamen (optional)
$configPath = Join-Path $gpoScriptsPath "OldPrintServers.txt"
try {
    $OldPrintServers | Out-File -FilePath $configPath -Encoding UTF8 -Force
    Write-Host "[OK] Konfigurationsdatei erstellt: $configPath" -ForegroundColor Green
} catch {
    Write-Host "[WARNUNG] Konnte Konfigurationsdatei nicht erstellen: $($_.Exception.Message)" -ForegroundColor Yellow
}

# Konfiguriere GPO: Script bei Anmeldung ausführen
try {
    # Erstelle Script-Befehl mit Parametern
    $serversParam = ($OldPrintServers | ForEach-Object { "-OldPrintServers '$_'" }) -join ' '
    $scriptCommand = "powershell.exe -ExecutionPolicy Bypass -WindowStyle Hidden -File `"$targetScriptPath`" $serversParam"
...
Write-Host "Alte Druckerserver: $($OldPrintServers -join ', ')" -ForegroundColor Green
Write-Host ""
Write-Host "HINWEIS: Das Script entfernt alle Drucker, die von den konfigurierten" -ForegroundColor Yellow
Write-Host "        alten Druckerservern stammen." -ForegroundColor Yellow
